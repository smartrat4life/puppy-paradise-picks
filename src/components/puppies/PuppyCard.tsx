...
const PuppyCard: React.FC<PuppyCardProps> = ({ puppy, ageInWeeks, onInquire, index = 0 }) => {
  ...
  return (
    <motion.div
      ...
      layout
    >
      <Card className="group overflow-hidden hover:shadow-lg transition-all duration-300 transform bg-gradient-to-br from-white to-amber-50 border border-amber-200 hover:border-teal-300 h-full flex flex-col">
        <motion.div 
        ...
        >
          ...
          >
            <Badge variant="secondary" className="bg-white/90 text-amber-800 font-bold text-xs">
              ${puppy.price}
            </Badge>
          </motion.div>
        ...
        <CardContent className="p-3 md:p-4 flex-1 flex flex-col">
          ...
          >
            <div className="flex items-center gap-1">
              ...
              <span>{ageInWeeks} week{ageInWeeks !== 1 ? 's' : ''}</span>
            </div>
            <div className="flex items-center gap-1">
          ...
          >
            <Button 
              className={`w-full text-xs md:text-sm ${puppy.status === 'available' 
